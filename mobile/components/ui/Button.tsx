...
export function Button({ label, onPress, variant = 'primary', size = 'md', loading = false, disabled = false, icon, style }: Props) {
  return (
    ...
    >
      {loading
        ? <ActivityIndicator color={variant === 'primary' ? Colors.textInverse : Colors.textPrimary} size="small" />
...
const styles = StyleSheet.create({
  ...
  variant_primary:   { backgroundColor: Colors.textPrimary, ...Shadow.brutalSm },
  variant_secondary: { backgroundColor: Colors.yellow,      ...Shadow.brutalSm },
