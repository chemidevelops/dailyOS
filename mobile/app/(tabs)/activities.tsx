...
function activityWeekDays(activity: ApiActivity): DayStatus[] {
  ...
  return (
    <Pressable onPress={() => onToggle(activity.id)} style={styles.activityCard}>
      <HStack style={{ justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <VStack gap="xs" style={{ flex: 1, marginRight: Spacing.md }}>
          <HStack gap="sm" style={{ alignItems: 'center' }}>
            <View style={[styles.colorBlock, { backgroundColor: activity.color }]} />
          ...
          <HStack gap="sm" style={{ alignItems: 'center', marginLeft: Spacing.sm + 10 }}>
            <Text variant="micro" color="tertiary">{doneDays}/{activity.target_per_week} esta semana</Text>
            {activeItem && (
      ...
      <HStack style={{ justifyContent: 'space-between', alignItems: 'center', marginTop: Spacing.sm }}>
        <WeekDots days={weekDays} color={activity.color} />
        <Text variant="micro" color="tertiary">{activity.duration_minutes > 0 ? `${activity.duration_minutes}m` : '—'}</Text>
      </HStack>
    </Pressable>
  )
}
...
function EmptyState() {
  return (
    <View style={styles.emptyWrap}>
      <Text variant="displayMedium" color="primary" style={{ marginBottom: Spacing.sm }}>Sin actividades</Text>
      <Text variant="body" color="secondary" style={{ textAlign: 'center' }}>
        Pulsa el + en la pestaña de hoy para añadir tu primera actividad.
      </Text>
    </View>
  )
}
...
export default function ActivitiesScreen() {
  ...
  const toggleActivity = async (id: number) => {
    ...
    setActivities(prev => prev.map(a => {
      if (a.id !== id) return a
      const logs = a.logs.filter(l => l.date !== TODAY_ISO)
      if (newStatus === 'done') {
        logs.push({ id: -1, activity_id: id, item_id: null, date: TODAY_ISO, status: 'done' })
      }
      return { ...a, logs }
    }))

    try {
      const saved = await api.activities.log(id, TODAY_ISO, newStatus)
      setActivities(prev => prev.map(a => {
        if (a.id !== id) return a
        const logs = a.logs.filter(l => l.date !== TODAY_ISO)
        logs.push(saved)
